app.post('/api/downloads/start', async (req, res) => {
    const { stream_id, stream_type, episodes, playlist_id } = req.body;
    ...
    // Jeśli nie podano playlist_id, spróbuj go znaleźć
    ...
    try {
        ...
        const stmt = db.prepare('INSERT OR IGNORE INTO downloads (stream_id, stream_type, playlist_id, episode_id, filename, status, worker_status) VALUES (?, ?, ?, ?, ?, ?, ?)');
        for (const episode of episodes) {
            await stmtRun(stmt, [stream_id, stream_type, finalPlaylistId, episode.id, episode.filename, 'queued', 'queued']);
        }
        stmt.finalize();
        await dbRun('COMMIT');
...
app.get('/api/downloads/status', async (req, res) => {
    try {
        ...
        const downloads = await dbAll(`
            SELECT 
                id, stream_id, stream_type, playlist_id, episode_id, filename, filepath,
...
app.post('/api/downloads/start', async (req, res) => {
    ...
    // Jeśli nie podano playlistconst express = require('express');
...
function initializeDb() {
    db.serialize(() => {
        ...
        // Dodaj brakujące kolumny do downloads jeśli nie istnieją
        db.all("PRAGMA table_info(downloads)", [], (err, columns) => {
            if (err) {
                console.error("Błąd sprawdzania struktury tabeli downloads:", err);
                return;
...
// --- API MEDIA (ZAKTUALIZOWANE Z OBSŁUGĄ PLAYLIST) ---
